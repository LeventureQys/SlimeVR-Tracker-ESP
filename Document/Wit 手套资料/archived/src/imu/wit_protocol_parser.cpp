...
bool WitProtocolParser::parseFrame(const QByteArray &frame)
{
    ...
    if (reg == 0x64) {
        data_.batteryPercent = batteryPercentForVoltage(signedValue(frame.at(4), frame.at(5)) / 100.0);
        return true;
    }
    if (reg == 0x40) {
    ...
    if (reg == 0x2e) {
        ...
        if ((version & 0x80000000U) != 0) {
            data_.firmwareVersion = QStringLiteral("%1.%2.%3")
                .arg((version >> 14) & 0x1ffffU).arg((version >> 8) & 0x3fU).arg(version & 0xffU);
        }
        return true;
    }
    return false;
