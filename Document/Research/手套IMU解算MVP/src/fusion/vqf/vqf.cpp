...
void VQF::updateAcc(const vqf_real_t acc[3])
{
    ...
    if (params.motionBiasEstEnabled || params.restBiasEstEnabled) {
        ...
        if (state.restDetected && params.restBiasEstEnabled) {
            ...
            std::fill(w, w+3, vqf_real_t(-1)); // disable update
        }
...
void VQF::setRestBiasEstEnabled(bool enabled)
{
    ...
    state.restDetected = false;
    std::fill(state.restLastSquaredDeviations, state.restLastSquaredDeviations + 3, vqf_real_t(0));
    state.restT = 0.0;
    std::fill(state.restLastGyrLp, state.restLastGyrLp + 3, vqf_real_t(0));
    std::fill(state.restGyrLpState, state.restGyrLpState + 3*2, NaN);
    std::fill(state.restLastAccLp, state.restLastAccLp + 3, vqf_real_t(0));
...
void VQF::resetState()
{
    ...
    std::fill(state.lastAccLp, state.lastAccLp+3, vqf_real_t(0));
    ...
    std::fill(state.bias, state.bias+3, vqf_real_t(0));
#ifndef VQF_NO_MOTION_BIAS_ESTIMATION
    ...
    state.magCandidateDip = 0.0;
    state.magCandidateT = 0.0;
    std::fill(state.magNormDip, state.magNormDip + 2, vqf_real_t(0));
    std::fill(state.magNormDipLpState, state.magNormDipLpState + 2*2, NaN);
}
...
bool VQF::matrix3Inv(const vqf_real_t in[9], vqf_real_t out[9])
{
    ...
    if (det >= -EPS && det <= EPS) {
        std::fill(out, out+9, vqf_real_t(0));
        return false;
    }
